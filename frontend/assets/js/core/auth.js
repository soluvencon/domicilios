// ============================================
// auth.js - Autenticación
// ============================================

var ADMIN_CREDENTIALS = {
    usuario: "admin",
    password: "admin123"
};

function mostrarModalLogin() {
    var modal = document.getElementById("modal-login");
    if (modal) modal.style.display = "flex";
}

function cerrarModalLogin() {
    var modal = document.getElementById("modal-login");
    if (modal) modal.style.display = "none";
    var usuarioInput = document.getElementById("login-nombre");
    var passInput = document.getElementById("login-password");
    if (usuarioInput) usuarioInput.value = "";
    if (passInput) passInput.value = "";
}

function validarLogin() {
    var nombre = document.getElementById("login-nombre").value.trim();
    var password = document.getElementById("login-password").value.trim();

    if (!nombre || !password) {
        alert("Ingrese usuario y contraseña");
        return;
    }

    // Verificar si es admin
    if (nombre === ADMIN_CREDENTIALS.usuario && password === ADMIN_CREDENTIALS.password) {
        guardarSesion("admin", "Administrador", 0);
        cerrarModalLogin();
        window.location.href = "admin.html";
        return;
    }

    // Consultar domiciliarios en la API
    fetch(API_URL + '?action=login&nombre=' + encodeURIComponent(nombre) + '&password=' + encodeURIComponent(password))
        .then(function(resp) { return resp.json(); })
        .then(function(data) {
            if (data.success) {
                guardarSesion("domiciliario", data.nombre, data.id);
                cerrarModalLogin();
                window.location.href = "domiciliario.html";
            } else {
                alert("❌ Usuario o contraseña incorrectos");
            }
        })
        .catch(function(error) {
            console.error("Error en login:", error);
            alert("Error al conectar con el servidor");
        });
}

function logout() {
    cerrarSesion();
}