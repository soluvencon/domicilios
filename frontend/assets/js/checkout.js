...
function cargarResumenPedido() {
    ...
    // ★ Mostrar tienda junto a cada producto
    container.innerHTML = carrito.map(item => `
        <div class="resumen-item">
            <span>${item.cantidad}x ${item.nombre}${item.tiendaNombre ? ` <small style="color:var(--secondary)">(${item.tiendaNombre})</small>` : ''} (${item.cantidadTipo} UND)</span>
...
async function procesarCheckout(e) {
    ...
    try {
        // ★ Incluir tiendaId y tiendaNombre en cada producto del JSON
        const productosJson = JSON.stringify(carrito.map(i => ({
            ...
            subtotal: i.subtotal,
            tiendaId: i.tiendaId || null,
            tiendaNombre: i.tiendaNombre || null
        })));
        ...
        if (data.success) {
            // ★ Extraer tiendas del carrito para el mensaje
            const tiendasSet = new Set();
            carrito.forEach(item => {
                if (item.tiendaNombre) tiendasSet.add(item.tiendaNombre);
            });
            const tiendasTexto = tiendasSet.size > 0 ? Array.from(tiendasSet).join(', ') : '';
            
            let mensaje = `🍕 *NUEVO PEDIDO - #${data.id}* 🍔\n\n`;
            mensaje += `🏪 *Tienda:* ${tiendasTexto || 'Varias'}\n\n`;
            ...
            carrito.forEach(item => {
                if (item.tiendaNombre && item.tiendaNombre !== tiendaActual) {
                    tiendaActual = item.tiendaNombre;
                    mensaje += `\n📦 *${tiendaActual}:*\n`;
                }
